...
$go_anyway = false;
$missing_continue = false;
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
  $go_anyway = validinput($_POST['go_anyway']) && $_POST['go_anyway'];
    if ($go_anyway || (validinput($_POST['name']) &&
    ...
    {
        $errors = true;
        $missing_continue = true;
        $error_message = "Some fields are missing or invalid. Please fix the ones highlighted in red, or select continue fix it on your own.";
    }
}
...
<body>
    ...
    <form action="#" method="POST" id="urp">
      ...
      <?php if ($error_message != "") echo '<p class="error message"><b>'.$error_message.'</b></br>'; ?>
      <?php if ($missing_continue == true) echo '<input type="checkbox" name="go_anyway" value="Continue Anyway" />Continue Anyway'; ?>
      <?php if ($error_message != "") echo '</p>'; ?>
